...
function handleMessage(message, sender) {
    ...
    let extension = new RegExp(/\w{3,4}$/gm).exec(url)[0]
...
function handleClick(tab) {
    ...
    if (url.indexOf(host) !== -1) {
        chrome.scripting.executeScript({
            ...
            files: [`content_scripts/getStoryUrl.js`],
        })
        chrome.scripting.executeScript({
            target: { tabId: id },
            files: [`content_scripts/downloadStory.js`],
        })
    }
}
...
async function fetchIcons() {
    let image_blob = await fetch(
        chrome.runtime.getURL('icons/icon_32_background.png')
    ).then((r) => r.blob())

    icon_background = await createImageBitmap(image_blob)
    image_blob = await fetch(
        chrome.runtime.getURL('icons/icon_32_arrow.png')
    ).then((r) => r.blob())
    icon_arrow = await createImageBitmap(image_blob)
    image_blob = await fetch(
        chrome.runtime.getURL('icons/icon_32_redbar.png')
    ).then((r) => r.blob())
